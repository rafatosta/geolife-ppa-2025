...
2. Execute o script abaixo para criar as tabelas usadas pelo atlas.
3. Em **Project Settings > API**, copie a **Project URL** e a chave pública (`anon` ou `publishable`). Configure-as no `.env` a partir do `.env.example`.
4. Reinicie `npm run dev` após alterar o `.env`.
...
create policy "catálogo público" on public.especies for all to anon using (true) with check (true);
create policy "regiões públicas" on public.regioes for all to anon using (true) with check (true);
create policy "observações públicas" on public.observacoes for all to anon using (true) with check (true);
```
