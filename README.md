...
2. Instale a CLI do Supabase e vincule o diretório ao projeto (`supabase link`).
3. Aplique `supabase/migrations/20260807000000_initial_schema.sql` com `npx supabase db push`.
4. Confirme no Storage que o bucket privado `activity-evidence` foi criado.
5. Para ambiente local, depois de autenticar contas de teste, execute `supabase db reset` ou aplique `supabase/seed.sql` manualmente. O seed não cria credenciais: ele só usa usuários já existentes em `auth.users`.

A migration cria:

- `profiles`, `reward_periods`, `rules`, `rule_weekdays`, `occurrences`, `submissions`, `submission_photos` e `ledger`;
- perfil `user/pending` automático no primeiro login;
- RLS que limita pendentes ao próprio perfil, usuários ativos aos próprios dados e administradores às operações de gestão;
- RPCs seguras para status de conta, revisão de submissão, penalidade e resumo financeiro;
- bucket privado e políticas por prefixo do usuário.

### Contrato CRUD

- perfis não são apagados pela interface: o administrador aprova, rejeita, bloqueia ou reativa;
- regras podem ser criadas, consultadas e editadas; a remoção é uma desativação que preserva ocorrências concluídas e o histórico;
- ao editar uma regra, somente ocorrências futuras ainda pendentes são regeneradas; registros já processados permanecem intactos;
- metas mensais podem ser criadas, listadas e editadas; a exclusão é recusada quando já existem lançamentos no ledger;
- ocorrências são encerradas por transição de estado (`completed`, `missed` ou `justified`), nunca por exclusão do histórico;
- evidências podem ser adicionadas e removidas enquanto a submissão estiver em rascunho ou rejeitada;
- o ledger é somente leitura no frontend e continua sendo a fonte auditável dos descontos.

### Bootstrap do primeiro administrador

Depois que a primeira pessoa criar a conta, execute no SQL Editor do Supabase, substituindo o e-mail:
...
Esse é um passo manual deliberado. Não existe autoelevação no cliente e usuários comuns não têm política de atualização de `role` ou `status`.
